...
import {convert25To18BigDecimal, convert25To18Decimals, getIntervalStartTime} from "./utils";
import {getOrCreateCandlestick, updateCandlestick} from "./candlestick"
import {calculate24HourMetrics, calculateLastHourChange} from "./priceAnalysis";
...
export function processSync(event: Sync): void {
    ...
    if (tokenSale == null) {
        log.error("TokenSale not found for token {}", [event.params.token.toHexString()]);
        return;
    }
    if (tokenSale) {
        tokenSale.tokenBalance = event.params.tokenReserve.plus(INITIAL_VIRTUAL_TOKEN_TO_ADD)
        tokenSale.vTokenBalance = event.params.tokenReserve.plus(INITIAL_VIRTUAL_TOKEN_TO_ADD)
        tokenSale.vBNBBalance = event.params.bnbReserve.plus(INITIAL_VIRTUAL_BNB_BALANCE)
        tokenSale.BNBBalance = event.params.bnbReserve
        tokenSale.lastPrice = convert25To18Decimals(event.params.endPrice)
        tokenSale.bnbMarketCap = BigInt.fromString(INITIAL_REAL_TOKEN_BALANCE.times(tokenSale.lastPrice).toBigDecimal().div(DECIMAL_FACTOR.toBigDecimal()).truncate(0).toString())
        tokenSale.totalTokensSold = TOKEN_FOR_SALE_BALANCE.minus(event.params.tokenReserve)
        ...
        tokenSale.SoldPercentage = soldTokensBD.div(totalTokensForSaleBD).times(HUNDRED_BD)
        ...
        for (let i = 0; i < INTERVALS.length; i++) {
            ...
            let candlestickId = event.params.token.toHexString() + "-" + interval + "-" + timestamp.toString()
