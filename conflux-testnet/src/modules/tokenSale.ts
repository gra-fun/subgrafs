import {TokenSaleConfig, TokenSaleCreated, TokenSaleFinished} from "../../generated/TokenSaleFactory/TokenSaleFactory";
import {PancakeSwapPool, TokenSale} from "../../generated/schema";
import {BigDecimal, BigInt, log} from "@graphprotocol/graph-ts";
import {predictPancakePoolV3Address, getPoolKey, calculateReferrerCommission} from "./utils"
...
import {
    ...
    ZERO,
    ZERO_BD, INITIAL_PRICE, DECIMAL_FACTOR,
    WBNB_ADDRESS, DEPLOY_FEE, ZERO_ADDRESS,
} from "./constants";
...
export function processTokenSaleCreated(event: TokenSaleCreated): void {
    ...
    let creator = getOrCreateUser(event.transaction.from.toHexString(), true, event.block.timestamp);

    creator.createdTokenSalesCount = creator.createdTokenSalesCount.plus(BigInt.fromI32(1));
    creator.save();

    log.info("New TokenSale created for token {}", [event.params.token.toHexString()]);
    ...
    tokenSale.bnbMarketCap = BigInt.fromString(INITIAL_REAL_TOKEN_BALANCE.times(INITIAL_PRICE).toBigDecimal().div(DECIMAL_FACTOR.toBigDecimal()).truncate(0).toString())
...
export function processTokenSaleFinished(event: TokenSaleFinished): void {
    ...
    tokenSale.vTokenBalance = tokenSale.vTokenBalance.minus(INITIAL_VIRTUAL_TOKEN_TO_ADD)
    tokenSale.vBNBBalance = tokenSale.vBNBBalance.minus(event.params.bnbReserve)
    tokenSale.tokenBalance = tokenSale.tokenBalance.minus(INITIAL_VIRTUAL_TOKEN_TO_ADD)
